...
管理台的「儲存位置」可以把**鏡像**與**匯出**資料夾指到別處,例如隨身碟 —— 那才是真正離開這台
機器的一份副本(三份預設檔案都在同一顆磁碟上,防的是誤刪與檔案損毀,不是硬碟故障)。

**主要紀錄的位置不開放更改。** 它必須永遠掛載、永遠可寫;若指向隨身碟而中途鬆脫,山上這台
機器會靜默地停止記錄,而現場沒有人會發現。鏡像則是冗餘 —— 只要有一份 JSONL 寫成功就算記錄
成功,鏡像掉了只會在畫面底部顯示警告。

選擇資料夾時會先**實際寫一個測試檔**再接受,因為「資料夾存在」不等於「寫得進去」:隨身碟可能
是唯讀的,網路磁碟可能掛載了但沒有寫入權限。更改後既有紀錄會**整份複製**過去,不是只從那一刻
開始記。設定的鏡像資料夾若在啟動時建不起來(隨身碟忘了插),程式照常啟動並記錄到主檔,只是
帶著警告。
