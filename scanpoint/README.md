...
Windows 掃描與 PIN 畫面會從 Flutter 視窗**暫時移除 IME 輸入內容**,避免已啟用的注音、拼音或
倉頡先把讀卡機按鍵轉成 `VK_PROCESSKEY`。進入管理設定時會恢復 IME,所以站點名稱仍可輸入中文。

再往下一層,解碼時讀的是 `KeyEvent.physicalKey`(實體按鍵位置的 USB HID code),自己對照
US-QWERTY 表換成字元 —— 而不是讀系統給的字元。因此掃描模式同時免疫三件事:
...
[windows/runner/kiosk_lock.cpp](windows/runner/kiosk_lock.cpp)
...
沒有資料庫。每一筆逐筆 flush,同時寫進每一份副本。

**兩種紀錄:**

| 檔案 | 內容 |
| --- | --- |
| `scans.jsonl` | 掃描成功的紀錄,一行一筆(含重複刷) |
| `scans-YYYY-MM-DD.csv` | 同上的試算表格式,依日期分檔,可直接開 |
| `events.jsonl` | 完整操作紀錄:失敗的讀取、管理員登入與被拒絕的 PIN、設定變更、儲存位置變更、匯出與上傳、啟動與離開 |
| `system_operations.csv` | 同上的試算表格式,每筆操作即時追加,可直接用 Excel 開啟 |

分成兩種是因為用途不同。掃描紀錄是**結果** —— 那些會變成某人完賽時間的列,乾淨到可以直接餵進
試算表。操作紀錄是**過程** —— 無人站點事後最常要回答的是「這位選手的打卡為什麼不見了」,而答案
通常在失敗與設定變更裡,不在成功紀錄裡。

**PIN 與上傳權杖永不寫入操作紀錄**,只留下「該欄位被改過」這件事;上傳只記 host 不記完整網址
(query string 可能夾帶密鑰)。這些檔案會被帶上隨身碟、可能進到共用試算表。操作紀錄的寫入
**永不拋例外** —— 因為日誌寫不出來而讓站點停止記錄,是很荒謬的輸掉比賽的方式。

**兩份固定副本,另可加一份額外備份；每份都有完整紀錄:**
...
管理台的「儲存位置」可以指定一個**額外備份資料夾**,例如隨身碟。那是兩份固定副本之外**多寫的
第三份** —— 固定位置照常寫,不會被取代。隨身碟拔掉時,原本的備援完全不受影響。
...
按下「匯出全部紀錄」後才選擇目的地；每次匯出都會在所選資料夾建立一個帶時間戳的完整快照。
...
要求操作員輸入站點編號、實際名稱與新的管理 PIN，並再次輸入 PIN 確認；儲存成功前不能進入掃描畫面。此畫面會暫停原生鍵盤阻擋並恢復
